...
> [지난 콘서트 티켓 예매 아키텍처 글](/posts/concert-ticket-reservation-architecture/)을 정리하면서, 좌석이 아니라 "상품"을 파는 이커머스는 구조가 어떻게 다를지 궁금해져서 자료를 찾아보고 정리한 학습 노트입니다. 다만 이번엔 개념 정리로 끝내지 않고, **재고 동시성 제어**와 **데이터 정합성(Saga+Outbox)** 부분은 실제로 작은 MSA로 구현해서 오버셀링 테스트까지 통과시켰습니다 — 코드는 [github.com/yoonxjoong/ecommerce-msa](https://github.com/yoonxjoong/ecommerce-msa)에 공개해뒀습니다. 반대로 **트래픽 대응(캐싱/대기열/Rate Limiting)** 부분은 아직 코드로 옮기지 못했고 여전히 설계 수준입니다 — 아래 각 섹션마다 어디까지가 실제로 검증됐고 어디부터가 설계인지 표시해뒀습니다. 틀린 부분이나 더 나은 방법이 있다면 언제든 알려주세요.
...
점선 없이 실선으로 그린 **CDN/캐시/장바구니 구간은 아직 설계만 한 부분**이고, **재고 서비스·주문 서비스·결제 서비스·재고 재동기화 배치는 실제로 구현하고 검증**했습니다. 핵심은 **상품 조회처럼 읽기가 압도적으로 많은 구간은 캐시로 흡수**하고, **주문 생성 이후 알림/정산 같은 후속 처리는 Kafka로 느슨하게 연결**한다는 점입니다. 아래에서 각 구간을 순서대로 살펴보겠습니다.

## 1. 트래픽 대응: 타임세일 오픈 순간 버티기

> **설계만 하고 아직 구현은 안 한 부분입니다.** 아래는 자료를 찾아보고 정리한 개념 수준의 설계입니다.
...
- **정적 리소스(이미지, JS/CSS)는 CDN으로 분리**해서, 세일 오픈 순간 원본 서버가 정적 요청 때문에 흔들리지 않게 합니다.
...
- 특정 인기 상품 하나에 재고 경합이 집중되는 "선착순 특가"류라면, 티켓 예매처럼 **가상 대기열**을 그 상품에 한해 앞단에 두는 것도 방법입니다. 다만 이커머스는 상품이 다양해서 전체 서비스에 대기열을 씌우기보다, 경합이 몰릴 게 뻔한 특정 이벤트 상품에만 국소적으로 적용하는 편이 자연스러워 보입니다.
...
한 가지 솔직히 짚을 점: 위에서 봤듯 Saga 보상은 REST 동기 응답만으로 이미 끝나기 때문에, **지금 이 Outbox→Kafka 파이프라인을 구독하는 컨슈머가 하나도 없습니다.** 발행까지는 검증했지만, 그 이벤트가 실제로 알림 서비스나 정산 서비스로 이어지는 건 아직 없는 "미래를 위한 확장 지점"인 상태입니다.
...
- **상품/카탈로그 서비스**: 상품 조회, Redis 캐시를 앞단에 둔 읽기 최적화
- **장바구니 서비스**: 사용자별 장바구니 상태 관리 (Redis)
- **재고 서비스** (구현): 재고 카운터 관리(Redis Lua 원자 연산), `product` 테이블(카탈로그+최초 시딩값)
- **주문 서비스** (구현): 주문 생성/상태 관리, Saga 오케스트레이터 (REST 동기 호출)
...
- **알림 서비스**: 주문/배송 상태 알림 발송 (Kafka `payment-events` 구독 대상이지만 아직 없음)
...
- **트래픽 대응(캐싱/대기열/Rate Limiting) 자체를 구현 안 함** — 이번엔 재고 동시성/정합성에 집중했고, 이 축은 여전히 설계만 있는 상태입니다.
- 장바구니에 담아둔 시점과 실제 결제 시점 사이 재고가 바뀌는 경우(장바구니 담을 땐 있었는데 결제 시점에 품절), 사용자 경험을 해치지 않으면서 처리하는 방법 — 애초에 장바구니 서비스 자체를 구현하지 않았습니다.
- 여러 상품이 한 주문에 섞여 있을 때(일부는 재고 확보, 일부는 품절) 부분 실패를 어떻게 다뤄야 하는지 — 지금 구현은 주문당 상품 1종만 지원해서 이 문제 자체를 피해갔습니다.
- Outbox → Kafka로 발행된 이벤트를 실제로 구독하는 컨슈머(알림/정산 서비스)가 없어서, 발행 파이프라인만 검증됐고 "그 다음"은 아직 없습니다.
- 실제 서비스들은 이 구조를 그대로 쓰기보다 트래픽 규모와 상품 특성(단일 SKU vs 옵션이 많은 상품)에 맞게 훨씬 단순화하거나 다르게 구성할 수도 있을 것 같습니다.

다음엔 트래픽 대응 축(캐싱, 대기열)을 실제로 구현해보거나, Kafka 이벤트를 구독하는 알림 서비스를 붙여서 파이프라인을 끝까지 이어보고 싶습니다.
