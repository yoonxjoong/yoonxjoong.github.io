...
> **캐싱(Cache-Aside)만 실제로 구현·검증했고, CDN/Rate Limiting/가상 대기열은 아직 설계 수준입니다.**
...
- **결제 직전 구간에는 Rate Limiting을 겁니다.** 상품 조회는 자유롭게 허용하되, "구매하기" 같은 쓰기 요청은 API Gateway 단에서 Token Bucket 등으로 제한해 재고 서비스·주문 서비스로 넘어가는 요청 양 자체를 조절합니다. *(설계만 함 — API Gateway 자체가 아직 없습니다.)*
...
- **트래픽 대응 중 캐싱만 구현, CDN/Rate Limiting/가상 대기열은 미구현** — 상품 조회 캐싱은 구현·검증했지만, 나머지는 여전히 설계만 있는 상태입니다.
...
다음엔 Rate Limiting이나 가상 대기열을 실제로 구현해보거나, Kafka 이벤트를 구독하는 알림 서비스를 붙여서 파이프라인을 끝까지 이어보고 싶습니다.
