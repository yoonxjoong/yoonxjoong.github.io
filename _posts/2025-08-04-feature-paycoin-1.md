...
현업에서 결제 모듈을 연동하는 업무를 담당하여 개발하고 있는 중에 기록하고자 하는 포스팅입니다.

현재 프로젝트는 Springboot + Vue.js 기반으로 구성되어 있고 새로운 결제 모듈을 추가 해야되는 상황입니다.

여기서 고민해봐야 될 상황은 결제 인증 후 콜백 응답을 받아 어떻게 처리해야될지가 중요하다고 생각이 듭니다.

다음과 같은 구조를 생각해봤고 구현을 해서 운영에 적용한 경험을 공유해보는 시간을 가져볼게요.
